...
library(gtools)
#Set working directory to the directory in which the results are contained in 
setwd("../../results/2020_7_7/paper_data1_rep107_nmut5/")
...
line_plot <-  ggplot(plot_data, aes(fill=species, color=species, size=species, x=time, y=transcript)) + geom_line(stat="identity", size=2) + ggtitle("Lowest rmse Genome") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(color = "black", size=1), axis.ticks = element_line(colour = "black", size = 1), axis.text=element_text(color="black",size=30,face="bold"), axis.title=element_text(size=30,face="bold"), plot.title=element_text(size = 25, face = "bold")) + scale_color_manual(values=c("#0072B2", "#E69F00", "#CC79A7")) + ylab('Transcript abundances')
ggsave("../../../data/figures/paper_data1_rep107_lowest.png", line_plot, width=10, height=10, limitsize=FALSE)
...
rmse_line_plot <- ggplot(rmse_data_acc, aes(x=Iteration, y=NRMSE)) + 
  ...
  xlab("Generation") + 
  ylab("Root mean square error (log scale)") + 
  panel_border() +
  scale_x_log10()
...
ggsave("../../../data/figures/paper_data1_rep107_rmse.png", rmse_line_plot, width=20, height=10, limitsize=FALSE)
...
rmse_data <- read.table("./final/rmse_data.tsv", header=TRUE)
rmse_data <- rmse_data %>% filter(Accepted=='yes')
min_rmse <- rmse_data$Iteration[rmse_data$rmse == min(rmse_data$rmse)]
```
