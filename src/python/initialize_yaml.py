...
def create_yaml(starting_file, gene_file):
    ...
    data = dict(
                ...
                region0 = dict(start=10, stop=gene_file['gene1']['start']),
    )
    ...
    for num_region in range(1, gene_file['num_genes']):
        data.update({'region{}'.format(num_region): dict(start=gene_file['gene{}'.format(num_region)]['stop'], stop=gene_file['gene{}'.format(num_region+1)]['start'])})
